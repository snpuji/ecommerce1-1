...
if ($id_kategori == 17 || $id_kategori == 18) {
    // Jika kategori fashion, pastikan semua stok ukuran diisi
    ...
    // Pastikan semua stok ukuran diisi
    if (empty($stok_small) || empty($stok_medium) || empty($stok_large) || empty($stok_extralarge) || empty($stok_doubleextralarge)) {
        echo '<div style="color:red">Please fill in all stock fields for fashion or related categories.</div>';
        exit;
    }
                    ...
                    <div class="mb-4">
                        ...
                        <select name="id_penjual" class="form-control" required>
                            ...
                            while ($row_penjual = mysqli_fetch_assoc($result_penjual)) {
                                echo "<option value='" . htmlspecialchars($row_penjual['id_penjual']) . "'>" . htmlspecialchars($row_penjual['nama_toko']) . "</option>";
                            }
                            ?>
                        </select>
                    </div>
